...
.request-badge{flex-shrink:0;color:var(--green);border:1px solid #d6e8df;background:#ecf5ef;border-radius:24px;padding:9px 15px;font-size:12px}
...
.request-form .request-field input,.request-form .request-field select{display:block;width:100%;min-width:0;height:45px;border:1px solid #dce5e0;border-radius:8px!important;background:#fcfdfc;color:#243d34;padding:10px 12px;font-size:14px;line-height:1.5;box-shadow:none;transition:border-color .15s,box-shadow .15s}
.request-form .request-field input::placeholder{color:#9aa6a1;font-size:13px}
...
@media(max-width:600px){.request-form{padding:22px 14px 30px}.request-intro{display:block}.request-intro h1{font-size:24px}.request-badge{display:inline-block;margin-top:13px}.request-section{padding:18px 16px}.request-grid{grid-template-columns:1fr;padding:19px 16px;gap:17px}.request-wide{grid-column:auto}.equipment-list{padding:0 16px}.equipment-row{grid-template-columns:24px minmax(0,1fr) minmax(0,1fr);gap:13px 10px}.equipment-row .equipment-choice{grid-column:2/-1}.equipment-row .equipment-amount{grid-column:2}.equipment-row .equipment-note{grid-column:2/-1}.equipment-number{margin-top:32px}.request-actions{flex-direction:column;align-items:stretch}.request-submit{width:100%}.request-form .request-field input,.request-form .request-field select{font-size:16px}}
</style>
</head>
...
<div class="request-intro"><div><div class="request-eyebrow">LIBRARY · EQUIPMENT REQUEST</div><h1>แบบฟอร์มขอใช้ครุภัณฑ์</h1><p>กรอกข้อมูลผู้ขอใช้และเลือกรายการที่ต้องการให้ครบถ้วน</p></div><span class="request-badge"><i class="fa fa-check-circle" aria-hidden="true"></i> ไม่ต้องเข้าสู่ระบบ</span></div>
<section class="request-card" aria-labelledby="applicant-title"><div class="request-section"><span class="request-step">01</span><div><h2 id="applicant-title">ข้อมูลผู้ขอใช้</h2><p>ข้อมูลติดต่อสำหรับประสานงานการขอใช้ครุภัณฑ์</p></div></div><div class="request-grid">
<div class="request-field"><label for="txt_name">ชื่อ <span class="required-mark">*</span></label><input id="txt_name" name="txt_name" type="text" class="validate[required]" placeholder="กรอกชื่อ" required maxlength="50" autocomplete="given-name"></div><div class="request-field"><label for="txt_surname">นามสกุล <span class="required-mark">*</span></label><input id="txt_surname" name="txt_surname" type="text" class="validate[required]" placeholder="กรอกนามสกุล" required maxlength="50" autocomplete="family-name"></div><div class="request-field request-wide"><label for="txt_department">หน่วยงาน / สังกัด</label><input id="txt_department" name="txt_department" type="text" class="" placeholder="ชื่อหน่วยงานหรือสังกัด"  maxlength="200" autocomplete="organization"></div><div class="request-field"><label for="txt_tel">เบอร์โทรศัพท์ <span class="required-mark">*</span></label><input id="txt_tel" name="txt_tel" type="text" class="validate[required]" placeholder="เบอร์โทรศัพท์ที่ติดต่อได้" required maxlength="30" autocomplete="tel" inputmode="tel"></div><div class="request-field"><label for="txt_email">อีเมล</label><input id="txt_email" name="txt_email" type="text" class="" placeholder="example@email.com"  autocomplete="email" inputmode="email"></div></div></section><section class="request-card" aria-labelledby="period-title"><div class="request-section"><span class="request-step">02</span><div><h2 id="period-title">รายละเอียดการขอใช้</h2><p>ระบุวัตถุประสงค์และช่วงเวลาที่ต้องการใช้งาน</p></div></div><div class="request-grid"><div class="request-field request-wide"><label for="txt_obj">วัตถุประสงค์ <span class="required-mark">*</span></label><input id="txt_obj" name="txt_obj" type="text" class="validate[required]" placeholder="เช่น ใช้ประกอบการจัดกิจกรรมหรือการประชุม" required maxlength="250"></div><div class="request-field"><label for="datepicker"><i class="fa fa-calendar" aria-hidden="true"></i> วันที่เริ่มใช้ <span class="required-mark">*</span></label><input type="text" id="datepicker" name="txt_date_start" class="validate[required]" placeholder="วัน/เดือน/ปี ค.ศ." autocomplete="off" required><p class="request-help">เลือกวันที่จากปฏิทิน</p></div><div class="request-field"><label for="datepicker2"><i class="fa fa-calendar" aria-hidden="true"></i> วันที่ส่งคืน <span class="required-mark">*</span></label><input type="text" id="datepicker2" name="txt_date_end" class="validate[required]" placeholder="วัน/เดือน/ปี ค.ศ." autocomplete="off" required><p class="request-help">เลือกวันที่จากปฏิทิน</p></div></div></section><section class="request-card" aria-labelledby="equipment-title"><div class="request-section"><span class="request-step">03</span><div><h2 id="equipment-title">รายการครุภัณฑ์</h2><p>เลือกได้สูงสุด 5 รายการ โดยกรอกรายการแรกเป็นอย่างน้อย</p></div></div><div class="equipment-list">
<?php
$request_equipment = [];
...
<div class="request-field equipment-choice"><label for="equipment-<?php echo $item; ?>">ครุภัณฑ์ <?php if ($item === 1): ?><span class="required-mark">*</span><?php endif; ?></label><select id="equipment-<?php echo $item; ?>" name="txt_title<?php echo $item; ?>" <?php if ($item === 1) echo 'class="validate[required]" required'; ?>><option value="">เลือกรายการครุภัณฑ์</option><?php foreach ($request_equipment as $equipment): ?><option value="<?php echo htmlspecialchars((string)$equipment['id'], ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($equipment['lda_list'], ENT_QUOTES, 'UTF-8'); ?></option><?php endforeach; ?></select></div>
<div class="request-field equipment-amount"><label for="amount-<?php echo $item; ?>">จำนวน <?php if ($item === 1): ?><span class="required-mark">*</span><?php endif; ?></label><input id="amount-<?php echo $item; ?>" name="txt_amount<?php echo $item; ?>" type="text" inputmode="numeric" maxlength="2" placeholder="0" onkeypress="checknumber()" <?php if ($item === 1) echo 'class="validate[required]" required'; ?>></div>
<div class="request-field"><label for="unit-<?php echo $item; ?>">หน่วย</label><input id="unit-<?php echo $item; ?>" name="txt_unit<?php echo $item; ?>" type="text" maxlength="20" placeholder="เช่น ชิ้น / เครื่อง"></div>
<div class="request-field equipment-note"><label for="note-<?php echo $item; ?>">หมายเหตุ</label><input id="note-<?php echo $item; ?>" name="txt_note<?php echo $item; ?>" type="text" placeholder="รายละเอียดเพิ่มเติม (ถ้ามี)"></div></div>
<?php endfor; ?>
</div></section><div class="request-actions"><p><span class="required-mark">*</span> ช่องที่ต้องกรอก<br>กรุณาตรวจสอบข้อมูลก่อนส่งคำขอ</p><button type="submit" class="request-submit">ส่งคำขอใช้ครุภัณฑ์ <i class="fa fa-arrow-right" aria-hidden="true"></i></button></div></form>
