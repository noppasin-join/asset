                ...
                <div class="row">
                    <div class="col-sm-12 col-xs-12">
                            ...
                            <td  colspan="15">
															...
															<?php
															$sql_def_number=ams_sql("SELECT * FROM data_number_check where year_budget=? and number_etc='0' and id_member_check=? order by number_check", ["$year_budget", "$id_member"]);
															...
															?>
																						...
																						<div class="pull-left">
																							<?php
																							$sql_show_number=ams_sql("SELECT * FROM data_number_check where year_budget=? and id_member_check=? and number_etc='0' group by number_check order by number_check", ["$year_budget", "$id_member"]);
                                          ...
                                          <?php
																					...
																					$query_year=ams_query($link,$sql_year);
																						...
																						$result_year=mysqli_fetch_array($query_year);
																							...
																							$stt_am=$staff_amount+1;

																					if($var_page=="") {
																						if($choose_status_main!="") {
																							if($choose_locate_main!="") {
																								$q=ams_sql("SELECT * FROM data_staff_choose where year_budget=? and id_member_choose=? and status=? and id_location_old=? order by barcode", ["$year_budget", "$id_member", "$choose_status_main", "$choose_locate_main"]);
																							} else {
																								$q=ams_sql("SELECT * FROM data_staff_choose where year_budget=? and id_member_choose=? and status=?  order by barcode", ["$year_budget", "$id_member", "$choose_status_main"]);
																							}
																						} else {
																							if($choose_locate_main!="") {
																								$q=ams_sql("SELECT * FROM data_staff_choose where year_budget=? and id_member_choose=? and id_location_old=?  order by barcode", ["$year_budget", "$id_member", "$choose_locate_main"]);
																							} else {
																								$q=ams_sql("SELECT * FROM data_staff_choose where year_budget=? and id_member_choose=?  order by barcode", ["$year_budget", "$id_member"]);
																							}
																						}
																					} else {
																						if($choose_status_main!="") {
																							if($choose_locate_main!="") {
																								$q=ams_sql("SELECT * FROM data_staff_choose where year_budget=? and number_check=? and status=? and id_location_old=? or number_check=? and id_member_choose=? order by barcode", ["$year_budget", "$var_page", "$choose_status_main", "$choose_locate_main", "$stt_am", "$id_member_check_ch"]);
																							} else {
																								$q=ams_sql("SELECT * FROM data_staff_choose where year_budget=? and number_check=? and status=? or number_check=? and id_member_choose=? order by barcode", ["$year_budget", "$var_page", "$choose_status_main", "$stt_am", "$id_member_check_ch"]);
																							}
																						} else {
																							if($choose_locate_main!="") {
																								$q=ams_sql("SELECT * FROM data_staff_choose where year_budget=? and number_check=? and id_location_old=? or number_check=? and id_member_choose=? order by barcode", ["$year_budget", "$var_page", "$choose_locate_main", "$stt_am", "$id_member_check_ch"]);
																							} else {
																								$q=ams_sql("SELECT * FROM data_staff_choose where year_budget=? and number_check=? or number_check=? and id_member_choose=? order by barcode", ["$year_budget", "$var_page", "$stt_am", "$id_member_check_ch"]);
																							}
																						}
																					}
																					...
																					<tr>
																						...
																						<td class="center font_brown hidden-1000" style="vertical-align:middle;">
																							<?php if($file_img!="") { ?>
																							<a  href="detail_data_pic.php?id=<?php echo $data_det; ?>" class="fancybox fancybox.ajax"><i class="ace-icon fa fa-picture-o bigger-130"></i></a>
																							<?php } else { echo "-"; }?>
																						</td>
                                            ...
                                            <?php if ($lda_status=="1") { ?>
                                              	<td class="center hidden-1000" bgcolor="#49ac8b" style="vertical-align:middle;"><font color="#FFFFFF">ใช้งานปกติ</font> </td><?php } elseif($lda_status=="2") { ?>
																								...
																								<td class="center hidden-1000" bgcolor="#d15b47" style="vertical-align:middle;"><font color="#FFFFFF">สภาพปกติ ไม่จำเป็นต้องใช้งาน</font></td><?php } elseif($lda_status=="8") { ?>
																								<td class="center hidden-1000" bgcolor="#d15b47" style="vertical-align:middle;"><font color="#FFFFFF">รอจำหน่ายออก</font></td><?php } ?>
                                            </td>
