...
$urlquery_str2=$_POST['urlquery_str2'] ?? '';
		...
		where id=?", ["$var_trim", "$choose_location_new", "$var_note", "$id_member", "$choose_status", "$day/$month/$year", "$time_log", "$id_data"]);
		$qr_number=ams_query($link,$sql_data1) or die ("Error Check");

		$sql_data2 = ams_sql("UPDATE data_lda set lda_status=?,id_location=?,name_use=?,note=?,
		id_member_update=?,date_update=?,time_update=? where id=?", ["$choose_status", "$choose_location_new", "$var_trim", "$var_note", "$id_member", "$day/$month/$year", "$time_log", "$id_data_lda"]);
		$qr_number=ams_query($link,$sql_data2) or die ("Error Check");
