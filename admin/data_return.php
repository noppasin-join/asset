...
$sql_br2=ams_sql("SELECT * FROM data_take  where year_budget=?", ["$year_budget"]);
...
$i_app=0;
	while($i_app<$num_br2)
	{
		...
		$id_app=$rs_app['id'];
				$sql_app_list=ams_sql("SELECT * FROM data_take_list  where id_data_take=? and status='1'", ["$id_app"]);
				$qr_app_list=ams_query($link,$sql_app_list);
				$num_app_list=mysqli_num_rows($qr_app_list);
					if($num_app_list=="0") {
						$sql_approve_update = ams_sql("update data_take set status_approve='2' where id=?", ["$id_app"]);
...
<style> body { font-family: sarabun; } </style>
				...
				function page_navigator2($before_p2,$plus_p2,$total2,$total_p2,$chk_page2,$j2,$choose_year,$txt_name,$var_page,$g){
				...
				$lt_page=$total_p2-4;
				if($chk_page2>0){
				echo "<a  href='?s_page2=$pPrev&urlquery_str2=".$urlquery_str2."&&radiobutton=2&&j2=1&&choose_year=$choose_year&&txt_name=$txt_name&&var_page=$var_page&&g=1' class='naviPN'><<</a>";
				}
				if($total_p2>=11){
				if($chk_page2>=4){
				echo "<a $nClass href='?s_page2=0&urlquery_str2=".$urlquery_str2."&&radiobutton=2&&j2=1&&choose_year=$choose_year&&txt_name=$txt_name&&var_page=$var_page&&g=1'>1</a><a class='SpaceC'>. . .</a>";
				}
				if($chk_page2<4){
				for($i=0;$i<$total_p2;$i++){
				$nClass=($chk_page2==$i)?"class='selectPage'":"";
				if($i<=4){
				echo "<a $nClass href='?s_page2=$i&urlquery_str2=".$urlquery_str2."&&radiobutton=2&&j2=1&&choose_year=$choose_year&&txt_name=$txt_name&&var_page=$var_page&&g=1'>".intval($i+1)."</a> ";
				}
				if($i==$total_p2-1 ){
				echo "<a class='SpaceC'>. . .</a><a $nClass href='?s_page2=$i&urlquery_str2=".$urlquery_str2."&&radiobutton=2&&j2=1&&choose_year=$choose_year&&txt_name=$txt_name&&var_page=$var_page&&g=1'>".intval($i+1)."</a> ";
				}
				}
				}
				...
				echo "<a $nClass href='?s_page2=".intval($st_page+$i)."&&radiobutton=2&&j2=1&&choose_year=$choose_year&&txt_name=$txt_name&&var_page=$var_page&&g=1'>".intval($st_page+$i+1)."</a> ";
				}
				for($i=0;$i<$total_p2;$i++){
				if($i==$total_p2-1 ){
				$nClass=($chk_page2==$i)?"class='selectPage'":"";
				...
				echo "<a $nClass href='?s_page2=".intval($lt_page+$i-1)."&&radiobutton=2&&j2=1&&choose_year=$choose_year&&txt_name=$txt_name&&var_page=$var_page&&g=1'>".intval($lt_page+$i)."</a> ";
				}
				}
				}else{
				for($i=0;$i<$total_p2;$i++){
				$nClass=($chk_page2==$i)?"class='selectPage'":"";
				echo "<a href='?s_page2=$i&urlquery_str2=".$urlquery_str2."&&radiobutton=2&&j2=1&&choose_year=$choose_year&&txt_name=$txt_name&&var_page=$var_page&&g=1' $nClass  >".intval($i+1)."</a> ";
				}
				}
				if($chk_page2<$total_p2-1){
				echo "<a href='?s_page2=$pNext&urlquery_str2=".$urlquery_str2."&&radiobutton=2&&j2=1&&choose_year=$choose_year&&txt_name=$txt_name&&var_page=$var_page&&g=1'  class='naviPN'>>></a>";
				}
				}
				?>
          ...
          <div class="main-content-inner">
            ...
            <div class="breadcrumbs" id="breadcrumbs">
              ...
              <ul class="breadcrumb">
                ...
                <li class="active">ข้อมูลการคืนวัสดุ/ครุภัณฑ์</li>
              </ul><!-- /.breadcrumb -->
            ...
            <div class="page-content">
                ...
                <div class="row">
                  <div class="alert9 alert-info font_brown">
                    <form name="form_choose" method="post" action="data_return.php">
                      <input type="hidden" name="j21" value="1">
                      ...
                      <i class="ace-icon fa fa-hand-o-right"></i> ปีงบประมาณ :
                        ...
                        <?php
        					 ...
        					 ?>
											...
											<font class="hidden-700">&nbsp;&nbsp;&nbsp; สถานะคืน :
                      <?php if ($txt_name != "") {  ?><select name="choose_status" style="background-color:#eeeeee" disabled>
                      <?php } else { ?><select name="choose_status" onChange="submit();" style="font-size:13px;"><?php } ?>
													...
													<?php if($choose_status=="1") { ?><option value="1" selected>คืนแล้ว</option><?php } else { ?><option value="1">คืนแล้ว</option><?php } ?>
													<?php if($choose_status=="0") { ?><option value="0" selected>ยังไม่คืน</option><?php } else { ?><option value="0">ยังไม่คืน</option><?php } ?>
                ...
                <div class="row">
                      ...
                      <div class="nav-search4" id="nav-search">
                        <form class="form-search">
                          ...
                          <input type="hidden" name="g" value="1">
													<input type="hidden" name="j2" value="1">
                        </form>
                      </div>
                  ...
                  <div class="row" style="padding:14px;"> </div>
							...
							if ($txt_name=="") {
									if ($choose_year=="") {
										$q="SELECT * FROM data_take where status_approve='1' group by id ";
									} else {
											if($var_page=="") {
												$q=ams_sql("SELECT * FROM data_take where year_budget=? and status_approve='1' group by id ", ["$choose_year"]);
											} else {
												$q=ams_sql("SELECT * FROM data_take where year_budget=? and month_submit=? and status_approve='1' group by id ", ["$choose_year", "$var_page"]);
											}
									}
							} else {
										$var_trim=trim($txt_name);
								    $q=ams_sql("SELECT * FROM data_take  where  name like ? and status_approve='1' order by id desc", ["%$var_trim%"]);
							}
							$qr=ams_query($link,$q);
                    ...
                    <div class="col-sm-12 col-xs-12">
                                    <div >
                                      <table class="table table-striped table-bordered table-hover">
                                        <thead>
                                          <tr>
                                            <td class="head_blue" colspan="15"> :: ข้อมูลการคืนวัสดุ/ครุภัณฑ์</td>
                                          </tr>
                                          <tr>
                                            ...
                                            <th class="center font_brown70 hidden-1100">จำนวนรายการ</th>
																						<th class="center font_brown70 hidden-1100">จำนวนคืน</th>
                                            <th class="center font_brown70 hidden-1200" >Manage</th>
                ...
                <?php
                                            ...
                                            <td class="center font_brown"><?php echo $rs['day_submit']; ?>/<?php echo $rs['month_submit']; ?>/<?php echo $rs['year_submit']; ?></td>
																						<td class="font_brown" align="left">
																							<a  href="detail_return.php?id=<?php echo $rs['id']; ?>" class="fancybox fancybox.ajax">
...
$pie2 = array_pad($pie2, 6, ''); $y_ch2=(is_numeric($pie2[0]) ? (int) $pie2[0] + 543 : '');
																						...
																						<td class="center font_brown hidden-1100">
																							<?php
																							$sql_num = ams_sql("select * from  data_take_list where id_data_take=? and status='1'", ["$rs[id]"]);
																							$qr_num=ams_query($link,$sql_num) or die ("เลือกข้อมูลไม่ได้8");
																						...
																						<td class="center font_brown hidden-1100">
																							<?php
																							$sql_num2 = ams_sql("select * from  data_take_list_more where id_data_take=? and status_return='1'", ["$rs[id]"]);
																							$qr_num2=ams_query($link,$sql_num2) or die ("เลือกข้อมูลไม่ได้9");
																							$num_num2=mysqli_num_rows($qr_num2);
																							?>
																							<?php echo number_format( $num_num2 ) ; ?>
																						</td>

																						<td class="center hidden-1200"> <a class="green" href="form_return.php?id=<?php echo $rs['id']; ?>&j2=1"><i class="ace-icon fa fa-cog bigger-130"></i></a></td>
                                          ...
                                          <?php } ?>
																	...
																	<?php   if ($total2 == 0 || $total_p2 == "1")  {  } else {  ?>
																		<div class="pull-right pagination9">
																			<li>
																				<?php page_navigator2($before_p2,$plus_p2,$total2,$total_p2,$chk_page2,$j2,$choose_year,$txt_name,$var_page,$g); ?>
																			</li>
																		</div>
																	<?php } ?>
