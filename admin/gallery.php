			...
			<div class="main-container" id="main-container">
				...
				<div class="main-content">
					...
					if($choose_year=="") {
							if($choose_category=="") {
									if($txt_name=="") {
											$q = "select * from  data_lda where lda_status='1' and file_img<>'' group by lda_list order by lda_list  ";
									} else {
										 $q = ams_sql("select * from  data_lda where lda_status='1' and lda_list like ? and file_img<>'' group by lda_list order by lda_list  ", ["%$txt_name%"]);
									}
							} else {
									if($txt_name=="") {
											$q = ams_sql("select * from  data_lda where lda_status='1' and id_category=? and file_img<>'' group by lda_list order by lda_list  ", ["$choose_category"]);
									} else {
											$q = ams_sql("select * from  data_lda where lda_status='1' and id_category=? and file_img<>'' and lda_list like ? group by lda_list order by lda_list  ", ["$choose_category", "%$txt_name%"]);
									}
							}
					} else {
						if($choose_category=="") {
									if($txt_name=="") {
											$q = ams_sql("select * from  data_lda where lda_status='1' and year_budget=? and file_img<>'' group by lda_list order by lda_list  ", ["$choose_year"]);
									} else {
											$q = ams_sql("select * from  data_lda where lda_status='1' and year_budget=? and file_img<>'' and lda_list like ? group by lda_list order by lda_list  ", ["$choose_year", "%$txt_name%"]);
									}
						} else {
									if($txt_name=="") {
											$q = ams_sql("select * from  data_lda where lda_status='1' and id_category=? and file_img<>'' and year_budget=? group by lda_list order by lda_list  ", ["$choose_category", "$choose_year"]);
									} else {
											$q = ams_sql("select * from  data_lda where lda_status='1' and id_category=? and file_img<>'' and year_budget=? and lda_list like ? group by lda_list order by lda_list  ", ["$choose_category", "$choose_year", "%$txt_name%"]);
									}
						}
					}
					...
					$total2=mysqli_num_rows($qr);
										...
										<div class="col-xs-12">
											<div class="nav-search4" id="nav-search">
												<form class="form-search">
													...
													<input type="text" placeholder="ชื่อครุภัณฑ์ ..." class="nav-search-input4"  autocomplete="off" name="txt_name" value="<?php echo $txt_name; ?>"  />
								...
								<div class="row">
									<div class="col-xs-12">
										...
										<div>
											<ul class="ace-thumbnails clearfix">
												...
												?>
												<li>
													<a href="file_img/<?php echo $rs['year_budget']; ?>/<?php echo $rs['barcode1']; ?>.jpg" title="ชื่อรายการ : <?php echo $rs['lda_list']; ?>" data-rel="colorbox">
														<img width="150" height="150" src="file_img/<?php echo $rs['year_budget']; ?>/<?php echo $rs['barcode1']; ?>.jpg" />
														<div class="text">
									...
									<div class="alert alert-block alert-danger row" id="success-alert">
										...
										<b class="black"><u>ไม่พบข้อมูลในระบบ</u></b>  &nbsp;&nbsp;กรุณาเลือกเงื่อนไขใหม่!
									</div>
									<?php } ?>
