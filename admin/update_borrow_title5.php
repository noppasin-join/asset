...
$amount=$_POST['amount'] ?? '';
$txt_title5=$_POST['txt_title5'] ?? '';
$trim_title5=trim($txt_title5);
...
if($amount=="1") {
	...
	$num_main_title=mysqli_num_rows($qr_main_title);
			...
			if($num_main_title=="0") {
				$sql_data1 = ams_sql("update data_take_list set title=? where id=?", ["$trim_title5", "$id_take_list"]);
				$qr_data1=ams_query($link,$sql_data1) or die ("Error Update Check5");
	...
	if($num_ch_title!="0") {
		...
		$sql_ch_title9 = ams_sql("select * from  data_take_list where title=? and id=? ", ["$trim_title5", "$id_take_list"]);
