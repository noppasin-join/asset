...
<?php include "phpqrcode/qrlib.php"; ?>
	...
	if ($choose_year=="") {
							...
							if ($txt_no=="" && $txt_title=="") {
											...
											if ($choose_category=="") {
																		if($choose_location == "") {
																			$q="SELECT * FROM data_lda where barcode3<>''  order by barcode1";
																		} else {
																			$q=ams_sql("SELECT * FROM data_lda where barcode3<>'' and id_location=?  order by barcode1", ["$choose_location"]);
																		}
											} else {

																		if($choose_location == "") {
																			$q=ams_sql("SELECT * FROM data_lda where barcode3<>'' and id_category=?  order by barcode1", ["$choose_category"]);
																		} else {
																			$q=ams_sql("SELECT * FROM data_lda where barcode3<>'' and id_category=? and id_location=?  order by barcode1", ["$choose_category", "$choose_location"]);
																		}
											}
																...
																$q=ams_sql("SELECT * FROM data_lda  where  barcode3<>'' and  lda_list like ?  order by barcode1  ", ["%$var_trim%"]);
							}
							...
							if ($txt_no=="" && $txt_title=="") {
											...
											if ($choose_category=="") {
																		if($choose_location == "") {
																			$q=ams_sql("SELECT * FROM data_lda where year_budget=? and barcode3<>''  order by barcode1", ["$choose_year"]);
																		} else {
																			$q=ams_sql("SELECT * FROM data_lda where year_budget=? and id_location=? and barcode3<>''  order by barcode1", ["$choose_year", "$choose_location"]);
																		}
											} else {

																		if($choose_location == "") {
																			$q=ams_sql("SELECT * FROM data_lda where year_budget=? and id_category=? and barcode3<>'' order by barcode1", ["$choose_year", "$choose_category"]);
																		} else {
																			$q=ams_sql("SELECT * FROM data_lda where year_budget=? and id_category=? and barcode3<>'' and id_location=?  order by barcode1", ["$choose_year", "$choose_category", "$choose_location"]);
																		}
											}
																...
																$var_trim=trim($txt_title);
																$q=ams_sql("SELECT * FROM data_lda  where  lda_list like ? and barcode3<>'' order by barcode1  ", ["%$var_trim%"]);
							}
...
<table border="0">
<tr>
<td><div style="width:140px;padding-top: 2px;font-size: 3px; "><?php echo $lda_list;?></div></td>
<td rowspan="2">
	<?php if($barcode3=="") { ?>
