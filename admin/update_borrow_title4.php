...
$amount=$_POST['amount'] ?? '';
$txt_title4=$_POST['txt_title4'] ?? '';
$trim_title4=trim($txt_title4);
...
if($amount=="1") {
	...
	$num_main_title=mysqli_num_rows($qr_main_title);
			...
			if($num_main_title=="0") {
				$sql_data1 = ams_sql("update data_take_list set title=? where id=?", ["$trim_title4", "$id_take_list"]);
				$qr_data1=ams_query($link,$sql_data1) or die ("Error Update Check4");
				echo "<meta http-equiv=\"Refresh\" content=\"0; URL=form_borrow.php?j=1&id=$id&#tbl3\">";
			} else {
	...
	if($num_ch_title!="0") {
		...
		$sql_ch_title9 = ams_sql("select * from  data_take_list where title=? and id=? ", ["$trim_title4", "$id_take_list"]);
