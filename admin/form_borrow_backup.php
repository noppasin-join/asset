            ...
            <div class="col-xs-12 col-sm-12 ">
              <div class="box box-success">
                ...
                <!-- /.box-body -->
						...
						<?php
						if($num_data>1) { $i_no2=1; }
						if($i_no2==1) {
							//2
							while ($i_no2<$num_data) {
                  ...
                  <div class="row" style="padding:3px;"> </div>
									...
									<div class="row">
												...
												<form action="update_borrow_title2.php" method="post" name="form_update">
													<input type="hidden" name="id_take_list2" value="<?php echo $id_take_list2;?>">
													<input type="hidden" name="id" value="<?php echo $id;?>">
												<div class="col-xs-12 col-sm-4">
												...
												<div class="col-xs-8 col-sm-2">
																<div class="input-group"><span class="input-group-addon" style="background-color:#eeeeee;font-size:12px;color:#797e7c;">หน่วยนับ</span>
																<input type="text" class="form-control" style="font-size:13px;background-color:#f1f9f2!important;height:34px;text-align:center;"
																 placeholder="หน่วยนับ"  maxlength="20" value="<?php echo $unit2; ?>" disabled>
																</div>
												<div class="row" style="padding:2px;"> </div>
												</div>
												<!-- 4 -->
												<div class="col-xs-12 col-sm-4">
																<div class="input-group"><span class="input-group-addon" style="background-color:#eeeeee;font-size:12px;color:#797e7c;">หมายเหตุ</span>
																		<input type="text" class="form-control"  disabled style="background-color:#f1f9f2!important;font-size:13px;height:34px;" value="<?php echo $note2; ?>"/>
																</div>
												</div>
									</div>
									<div class="row" style="padding:2px;"> </div>
									...
									<?php
												$sql_check_2 = ams_sql("select * from  data_lda where lda_list like ? and lda_status='1' and status_borrow='1' ", ["%$title2%"]);
												$qr_check_2=ams_query($link,$sql_check_2) or die ("เลือกข้อมูลไม่ได้");
												$num_check_2=mysqli_num_rows($qr_check_2);
									?>
									<?php if($num_check_2!="0") { ?>
									<div class="row">
												<!-- col 1 -->
												<div class="col-xs-12 col-sm-6">
																<div>
																...
																$i_check_21=1;
																$i_check_22=0;
																	while($i_check_21<$num_check)
																	{
																		$rs_check_2=mysqli_fetch_array($qr_check_2);
																		$id_br_2=$rs_check_2['id'];
																		$barcode1_2=$rs_check_2['barcode1'];
																		$lda_list_sh_2=$rs_check_2['lda_list'];
																		$status_return_2=$rs_check_2['status_return'];

																?>
																<tr>
																	<td class="center"><?php echo $i_check_21;?>.</td>
																	<td class="center"><?php echo $barcode1_2;?></td>
																	<td ><?php echo $lda_list_sh_2;?></td>
																	<?php if ($status_return_2=="1") { ?><td class="center" bgcolor="#49ac8b" width="12%"><font color="#FFFFFF">ว่าง</font> </td>
																<?php } elseif($status_return_2=="2") { ?><td class="center" bgcolor="#d15b47" width="12%"><font color="#FFFFFF">ไม่ว่าง</font></td>
																	<?php } ?>
																	<td class="center" width="6%">
																		<?php if($status_return_2=="1") { ?>
																		<a class="green" href="update_borrow_check_2.php?id=<?php echo $id; ?>&id_data_lda2=<?php echo $id_br_2; ?>
																			&id_take_list2=<?php echo $id_take_list2; ?>&amount2=<?php echo $amount2; ?>&barcode2=<?php echo $barcode1_2; ?>">
																			<i class="ace-icon fa fa-check bigger-130 blue"></i>
																		</a>
																	<?php } elseif($status_return_2=="2") { ?><i class="ace-icon fa fa-times bigger-120 red2"></i><?php } ?>
																	</td>
																</tr>
																<?php $i_check_21++; $i_check_22++; } ?>
																</table>
																</div>
												<div class="row" style="padding:2px;"> </div>
												</div>
												...
												<?php
												if($amount2!="1") {
												$sql_data_22 = ams_sql("select * from  data_take_list_more where id_data_take_list=? order by barcode ", ["$id_take_list2"]);
												$qr_data_22=ams_query($link,$sql_data_22) or die ("เลือกข้อมูลไม่ได้");
												$num_data_22=mysqli_num_rows($qr_data_22);
												?>
												<?php if($num_data_22!="0") { ?>
												...
												<div class="col-xs-12 col-sm-6">
																...
																$i_more_21=1;
																$i_more_22=0;
																	while($i_more_22<$num_data_22)
																	{
																		$rs_more_22=mysqli_fetch_array($qr_data_22);
																		$id_take_list_more_22=$rs_more_22['id'];
																		$id_data_lda_more_22=$rs_more_22['id_data_lda'];
																				$sql_bring_data_22 = ams_sql("select * from  data_lda where id=?", ["$id_data_lda_more_22"]);
																				$qr_bring_data_22=ams_query($link,$sql_bring_data_22) or die ("เลือกข้อมูลไม่ได้");
																				$rs_bring_22=mysqli_fetch_array($qr_bring_data_22);
																				$barcode_bring_22=$rs_bring_22['barcode1'];
																				$title_bring_22=$rs_bring_22['lda_list'];
																?>
																<tr>
																	<td class="center"><?php echo $i_more_21;?>.</td>
																	<td class="center"><?php echo $barcode_bring_22;?></td>
																	<td ><?php echo $title_bring_22;?></td>
																	<td class="center">
																		<a class="green" href="update_borrow_reply2.php?id=<?php echo $id; ?>&id_take_list=<?php echo $id_take_list2; ?>
																			&id_take_list_more=<?php echo $id_take_list_more_22; ?>&id_data_lda_more=<?php echo $id_data_lda_more_22; ?>">
																			<i class="ace-icon fa fa-mail-reply bigger-130 green"></i>
																		</a>
																	</td>
																</tr>
																<?php $i_more_21++; $i_more_22++; } ?>
																</table>
																</div>
													<div class="row" style="padding:2px;"> </div>
													</div>
												...
												<?php } elseif($amount2=="1") { ?>
													<?php
													$sql_data_21 = ams_sql("select * from  data_take_list where id=? and id_data_lda<>'' ", ["$id_take_list2"]);
													$qr_data_21=ams_query($link,$sql_data_21) or die ("เลือกข้อมูลไม่ได้");
													$num_data_21=mysqli_num_rows($qr_data_21);
													$rs_more_21=mysqli_fetch_array($qr_data_21);
													$id_data_lda_one_21=$rs_more_21['id_data_lda'];
															$sql_one_data_21 = ams_sql("select * from  data_lda where id=?", ["$id_data_lda_one_21"]);
															$qr_one_data_21=ams_query($link,$sql_one_data_21) or die ("เลือกข้อมูลไม่ได้");
															$rs_mor_one_21=mysqli_fetch_array($qr_one_data_21);
															$barcode_more_21=$rs_mor_one_21['barcode1'];
															$title_more_21=$rs_mor_one_21['lda_list'];

													?>
													<?php if($num_data_21!=0) { ?>
													<div class="col-xs-12 col-sm-6">
																	<div >
																	...
																	<tr>
																		<td class="center">1.</td>
																		<td class="center"><?php echo $barcode_more_21;?></td>
																		<td ><?php echo $title_more_21;?></td>
																		<td class="center">
																			<a class="green" href="update_borrow_reply2_1.php?id=<?php echo $id; ?>&id_take_list=<?php echo $id_take_list2; ?>&id_data_lda=<?php echo $id_data_lda_one_21; ?>">
																				<i class="ace-icon fa fa-mail-reply bigger-130 green"></i>
																			</a>
																		</td>
																	</tr>
...
e
					...
					<?php
					if( $num_data>2) { $i_no3=2; }
					if($i_no3==2) { echo "eee";

						while ($i_no3<$num_data) {
							$rs_data3=mysqli_fetch_array($qr_data);
							$id_take_list3=$rs_data3['id'];
							$title3=$rs_data3['title'];echo $title3;
